...
describe('CLI', () => {
  ...
  describe('command list', () => {
    it('lists the available CLI commands when none is provided', async () => {
      ...
      expect(loggerErrorSpy).toHaveBeenCalledWith(
        expect.stringContaining('test, preview, export, info')
      )
      // The browser sign-in `login` command is gone from the OTP-based flow, so
      // it must not be advertised as a top-level command.
      const availableCommandsCall = loggerErrorSpy.mock.calls.find(
        (call) =>
          typeof call[0] === 'string' &&
          (call[0] as string).startsWith('Available commands:')
      )
      expect(availableCommandsCall).toBeDefined()
      expect(availableCommandsCall?.[0]).not.toContain('login')
    })
  })

  describe('requireScreenCISecret', () => {
    it('returns the secret and prod api url when SCREENCI_SECRET is set', async () => {
      process.env.SCREENCI_SECRET = 'secret-from-env'

      const { requireScreenCISecret } = await import('./cli')
      const result = await requireScreenCISecret()

      expect(result.secret).toBe('secret-from-env')
      expect(result.apiUrl).toBe('https://api.screenci.com')
      expect(result.resolvedConfigPath).toContain('screenci.config.ts')
    })

    it('logs the secrets url and exits non-zero when SCREENCI_SECRET is missing', async () => {
      delete process.env.SCREENCI_SECRET

      const { requireScreenCISecret } = await import('./cli')

      await expect(requireScreenCISecret()).rejects.toThrow(
        'process.exit called'
      )
