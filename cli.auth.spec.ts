...
describe('CLI', () => {
  ...
  describe('record auth bootstrap', () => {
    it('reuses a stored valid link session', async () => {
      mockReadFile.mockImplementation(async (path: string | URL) => {
    ...
    it('replaces an invalid stored link session automatically', async () => {
      let storedLinkSession: string | null = JSON.stringify({
        token: 'stale-session',
        appUrl: 'https://app.screenci.com/cli-auth?session=stale-session',
        pollUrl:
          'https://api.screenci.com/cli-link/session?token=stale-session',
        createdAt: '2026-06-11T10:00:00.000Z',
        expiresAt: '2099-06-11T10:15:00.000Z',
        environment: 'prod',
        envFilePath: `${process.cwd()}/.env`,
      ...
      mockReadFile.mockImplementation(async (path: string | URL) => {
        const pathString = String(path)
        if (pathString.endsWith('link-session.json')) {
          if (storedLinkSession === null) {
            const error = new Error('ENOENT') as Error & { code?: string }
            error.code = 'ENOENT'
            throw error
          }
          return storedLinkSession
        }
        if (pathString.endsWith('screenci.config.ts')) {
          return "export default defineConfig({ projectName: 'Test Project' })"
        }
      ...
      mockFetch.mockImplementation(
        async (input: string | URL, init?: RequestInit) => {
          const url = String(input)
          if (url.includes('token=stale-session')) {
            return {
              ok: false,
              status: 404,
          ...
          if (url.endsWith('/cli-link/session') && init?.method === 'POST') {
            storedLinkSession = JSON.stringify({
              token: 'fresh-session',
              appUrl: 'https://app.screenci.com/cli-auth?session=fresh-session',
              pollUrl:
                'https://api.screenci.com/cli-link/session?token=fresh-session',
              createdAt: '2026-06-11T10:00:00.000Z',
              expiresAt: '2099-06-11T10:15:00.000Z',
              environment: 'prod',
              envFilePath: `${process.cwd()}/.env`,
          ...
          if (url.includes('token=fresh-session')) {
            return {
              ok: true,
              status: 200,
              json: vi.fn().mockResolvedValue({
      ...
      mockRmSync.mockImplementation((path: string) => {
        if (String(path).endsWith('link-session.json')) {
          storedLinkSession = null
        }
      })
      ...
      expect(mockWriteFile).toHaveBeenCalledWith(
        `${process.cwd()}/.screenci/link-session.json`,
        expect.stringContaining('"token": "fresh-session"')
      )
    })
