...
| `screenci merge-complete`         | Report that sources pulled by an "Add to repository" or "Add to CI" code were committed |
...
language**, **Re-record** / **Record all**, **Add to repository**, and
**Add to CI** buttons in the app produce a prompt that carries a one-time
setup code and points the agent at the brief for that button
(`screenci.com/add-project.md`, `/add-video.md`, `/add-screenshot.md`,
`/edit-video.md`, `/add-language.md`, `/record-video.md`,
`/merge-sources.md`, or `/add-to-ci.md`); the coding agent runs the command
in the repository of the app to record, or in an empty folder:
...
1. Exchanges the code for a project-scoped `SCREENCI_SECRET` and a personal
   and writes it into the workspace's env file. The
...
2. Reads the organisation's [AI context](/docs/guides/ai-context) (with the
   ...
   its `origin` is the configured repository or when it already holds the
   project's repository-managed `screenci/` workspace (a fork, or a
   repository whose URL nobody told ScreenCI), else a shallow clone in
...
3. Prepares the workspace, `screenci/` inside the repository when one exists
   ...
   - **Present, repository-managed (no `projectId`), same project name:** used
     as is; the agent commits its change to git. When that workspace sits in
     the clone under `.screenci/repo` (the command ran outside the
     repository), the agent records against the site instead of committing:
     `start` writes `SCREENCI_UPLOAD_SOURCES=1` into the env file so `preview`
     and `export` upload the changed scripts, and the web app then offers
     **Add to repository** for them.
   - **Present, another project:** refuses; use `--dir`.
   - **Merge code:** pulls the sources into `screenci/` inside the repository
     (over an existing repository workspace when ScreenCI holds newer
     sources), removes `projectId` from the config, and writes
   ...
   - **CI code:** mints a CI key (no person behind it, so its uploads show as
     "CI"), locates the repository workspace (pulling the sources into it
     first, like a merge code, when they still live in ScreenCI), lists the
     CI providers whose files the repository has (`.github/workflows`,
     `.gitlab-ci.yml`, `.circleci/config.yml`, `.buildkite`,
     `bitbucket-pipelines.yml`, `Jenkinsfile`, `azure-pipelines.yml`), and
     prints a brief that stores the key in the provider's secret store, adds
     the pipeline ([`ci-workflow`](#screenci-ci-workflow) for GitHub Actions,
     the [CI setup templates](/docs/ci-setup#other-providers) elsewhere), and
...
The last step of **Add to repository** (and of **Add to CI** when the
sources still lived in ScreenCI). Run it from the `screenci/` workspace
...
`screenci start`), a project with `uploadSources: true`, or a workspace whose
environment has `SCREENCI_UPLOAD_SOURCES=1` (a clone workspace prepared by
`screenci start`), `preview` and `export` upload the island's text sources
right before recording, and report the finished run afterwards:
