...
`edit` uploads under a local, anonymous trial session and prints the web
editor link, then you sign up to keep it. Recording an anonymous trial agrees
...
Opens the editing flow for exactly one video. `edit` syncs any queued web
edits into your `.screenci.ts` sources, re-records and uploads the video's
live preview if it is stale, prints the web editor deep link
(`<app>/project/<projectId>/video/<videoId>?editor`), and exits. Edits made
in the web editor while no machine is connected queue server-side and are
written into your sources by the next `screenci` command (`sync`, `test`,
`edit`, or `export`).

With `--watch`, the command instead stays running as the live code-sync
bridge: every web edit is written back into your sources within seconds, and
the editor can trigger a local record of the video on demand. It keeps
...
Source watching: while connected with `--watch`, `screenci edit` watches the test source files
...
create one by hand there and set it yourself). Run `screenci edit --watch` in
...
Without an account, no setup is needed: `screenci edit` runs under a local,
...
result back so the editor's pending queue clears. `test`, `export`, and
`edit` run the same sync automatically on start (`export` accepts
`--no-sync` to skip it), so `sync` is mostly useful to converge sources
