...
- pass `--poll-auth` in a non-interactive session to wait for sign-in instead of exiting: `record` prints the link, polls every 5 seconds, and continues recording automatically once sign-in completes
...
- `-c, --config <path>`
- `-v, --verbose`
- `--poll-auth` (non-interactive sessions: wait for sign-in, polling every 5s, then continue)
