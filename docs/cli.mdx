...
5. Decides where to record. The scripts name an address (`webServer.url` or
   ...
   it by hand: set `use.baseURL` in `screenci.config.ts` to the live site,
   remove the `webServer` block for this run, and turn any `page.goto` that
   spells out the local address into a path relative to the base URL. It
