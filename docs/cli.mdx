...
| `screenci context`                | Print what the agent knows about this project (site, sign-in, notes)           |
...
2. Reads what the project knows ([AI context](/docs/guides/ai-context): its
   site URL, whether it needs a sign-in, the notes for the agent) and
   locates the repository: the git checkout the
   command runs in, when there is one. Nothing is cloned; outside any
   repository the brief tells the agent to work from the site alone.
3. Prepares the workspace. Inside a repository it is the island the
   repository already holds for the project, wherever it sits (a monorepo
   keeps it under a package; the config's `projectId`, or its `projectName`
   for an island made by `screenci init`, says which project it belongs to),
   else the repository's `screenci/`; outside any repository, `./screenci`.
   Every `preview` and `export` uploads the workspace's text sources, and
   each version remembers the sources it was recorded from (see
   ...
   - **Absent, new project:** scaffolds the workspace like `init --yes`
     ...
     `screenci.config.ts`.
   - **Absent, existing project with sources on ScreenCI:** pulls them,
     installs dependencies and the Playwright browser, and installs the
     agent skills at the repository root. A code made from a video page
     (Edit, Add a language, Re-record of one video) pulls the sources of the
     version the person was viewing, so any version can be the starting
     point of an edit; the project-level codes pull the project's latest.
     Binary media is not pulled; recordings reuse the project's uploaded
     assets.
   - **Absent, existing project without sources:** scaffolds a fresh
     workspace for it. A code that addresses one video refuses instead: its
     script is only in the repository, so run the command there or pass
     `--dir`.
   - **Present, same `projectId`, or no `projectId` and the same project
     name:** used. For a code made from a video page, the version's sources
     are the starting point: outside a repository, files that differ are
     replaced with the version's and listed (every recording made there was
     uploaded, so earlier versions keep their own sources on ScreenCI);
     inside a repository, the repository's scripts win and the brief lists
     the files in which the version differs (`--force` replaces them with
     the version's, refused while the tree has uncommitted changes). An
     island that only names the project gets its `projectId` written in.
   - **Present, another project:** refuses and proposes `--dir
screenci-<project>`.
   ...
   - **CI code:** mints a CI key (no person behind it, so its uploads show as
     "CI"), needs the repository (the one the command runs in),
...
5. Decides where to record. The scripts name an address (`webServer.url` or
   `use.baseURL` in `screenci.config.ts`). When that is a dev server and
   this machine can run it (it already answers, or the command runs inside
   the repository and the agent may start the app), the recording goes there
   as usual. Otherwise the deployed site takes over: the prompt's app URL,
   else the context's site URL, else the site the chosen version was
   recorded against. The brief then tells the agent to point the scripts at
   it by hand: `video.use({ baseURL })` on the video it works on (the
   config and the other videos stay as they are), no `webServer` block for
   this run, and any `page.goto` that spells out the local address turned
   into a path relative to the base URL. It
   also says to check that the data the flow expects exists on the live site
   before recording, and to report a step that needs data rather than work
   around it. A teammate without the repository can therefore edit a video
   whose scripts an engineer pointed at `localhost`. The chosen address is
   probed; when
   nothing answers and nothing else is known, the brief says **STOP** and the
   command exits with code 2; see
   [Running the app locally](/docs/guides/ai-context#running-the-app-locally).
6. Prints a brief for the agent (the task, the starting point, the
   repository, the site, how to sign in, the team's notes, which script to
   edit, the commands to run)
   followed by one machine-readable JSON line (`{"status":"ready", ...}`, or
   `"stopped"` with a `stop` object). The line also carries
   `videoSourcePath` and how it was found (`videoSourceLocation`: `server`,
   `title`, `other-island`, `missing`), the `startingPoint` (the version and
   the files replaced or differing), and the `recordingTarget`
   (`configured`, `override` with the address to use, or `stop`).
...
- `--dir <path>` uses another workspace folder than the one `setup` would
  pick (it wins over the island found in the repository, also for Add to CI).
- `--force` replaces an existing workspace of this project with the sources
  the code starts from (the chosen version's, or the project's latest); local
  files that differ are overwritten and listed. Refused inside a repository
  whose tree has uncommitted changes.
...
Prints what the agent knows about the workspace's project
([AI context](/docs/guides/ai-context): the site URL, whether the site needs
a sign-in, the team's notes), whether a
...
report the finished run afterwards. Every version remembers the sources it
was recorded from: that is what the video page shows under "Sources", what
`screenci setup` pulls onto a machine without a workspace, and what an Edit
or Re-record prompt made from a version page starts from.
...
- reading what the project knows about itself with `context`
