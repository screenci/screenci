...
heuristic matching, so an applied edit is never a guess. Edits on actions
that have not been stamped yet (record once, then sync) print as agent-prompt
instructions instead, exactly as `screenci sync-prompt` would emit them, as
do loop executions, non-literal option values, narration and overlay content,
moved timestamps with offsets, presentation updates, and Studio options.
