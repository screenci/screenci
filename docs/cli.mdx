...
**Add project**, **Add video**, and **Edit** buttons in the app produce a
prompt that carries a one-time setup code and points the agent at the brief
for that button (`screenci.com/add-project.md`, `/add-video.md`, or
`/edit-video.md`); the coding agent runs the command in the repository of the
app to record, or in an empty folder:
...
2. Prepares `./screenci`:
   - **Absent, new project:** scaffolds the island like `init --yes` (agent
     ...
     `screenci.config.ts`.
   - **Absent, existing project:** pulls the project's latest uploaded
     sources, installs dependencies and the Playwright browser, and installs
     the agent skills at the repository root. Binary media is not pulled;
     recordings reuse the project's uploaded assets.
   ...
   - **Present, another project (or no `projectId`):** refuses; use `--dir`.
3. Prints a brief for the agent (the task, the app URL, which script to edit,
   the commands to run) followed by one machine-readable JSON line
   (`{"status":"started", ...}`).
