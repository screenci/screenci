...
[Anonymous Trial](/docs/guides/anonymous-trial)), the anon session token
doubles as the editor token, and the trial's editor page is editable while
your machine is connected. Signing up claims the trial: the running session
picks up your new `SCREENCI_SECRET` and a claim-minted `SCREENCI_EDIT_TOKEN`
(written into `screenci/.env`) and reconnects without restarting.
