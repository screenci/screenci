...
5. Decides where to record. The scripts name an address (`webServer.url` or
   ...
   recorded against. The brief then has the agent run every command with
   `SCREENCI_BASE_URL=<site>` (see [Configuration](/docs/reference/configuration#screenci_base_url)),
   which replaces `use.baseURL` and skips the `webServer`, so the config is
   left as it is and the same scripts record in both places. A teammate
   without the repository can therefore edit a video whose scripts an
   engineer pointed at `localhost`. The chosen address is probed; when
