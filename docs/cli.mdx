...
page (it is a placeholder, not a working token). With a token, `init` exchanges
it for your organization's `SCREENCI_SECRET` and writes it into `screenci/.env`,
so you can `screenci record` immediately on the free tier. The token is
single-use and short-lived; a used or expired token falls back to the manual
path below without failing the scaffold. You can also pass your `SCREENCI_SECRET`
directly instead of a token, and `init` writes it straight into `screenci/.env`.
...
- `--no-react` skips React overlay support (react/react-dom, JSX, the `.tsx`
  example)
