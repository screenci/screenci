...
| `screenci login`                   | Authenticate and save `SCREENCI_SECRET` for a project |
...
Authenticate ScreenCI for the current project and save `SCREENCI_SECRET` into
the configured `envFile` or project `.env`.
...
```bash
npx screenci login
npx screenci login --open
npx screenci login --config screenci.config.ts
```
...
pnpm exec screenci login --open
pnpm exec screenci login --config screenci.config.ts
```
...
```bash
yarn screenci login
yarn screenci login --open
yarn screenci login --config screenci.config.ts
```
...
- prints the ScreenCI login URL every time
- prompts before opening a browser by default
- opens the browser immediately when `--open` is passed
- saves `SCREENCI_SECRET` into the project env file after a successful callback
- if you want to set the secret manually, get it from [app.screenci.com/secrets](https://app.screenci.com/secrets) and add `SCREENCI_SECRET=...` to the configured `envFile` or project `.env`
...
- never opens a browser or starts login automatically
- requires `SCREENCI_SECRET`; run `screenci login` first or add it to the project env file
