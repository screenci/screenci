...
the next `screenci` command (`sync`, `test`, `preview`, or `export`).
...
Startup check: `screenci preview` keeps each recording's `data.json` on disk after
...
`preview` run the same sync automatically on start (`export` accepts
