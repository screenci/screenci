...
- `--sync`: auto-apply web editor edits to the recording sources while
  listening. Every few seconds the listener checks for new edits and applies
  what [`screenci sync`](#screenci-sync) can apply mechanically; a video whose
...
static analysis (the TypeScript parser), no agent involved. It can:
