...
- when `SCREENCI_SECRET` is missing in a non-interactive session (no terminal, or `SCREENCI_NONINTERACTIVE=1`), does not wait: prints the sign-in link and exits, so you can sign in and choose a plan in the browser, then rerun `record`, which detects the completed session and continues
