...
Each language exposes up to two URL sets:

- `static`: stable URLs that always follow the currently selected version
- `latestRecord`: this run's render status plus URLs pinned to it with
  `?record=<recordId>` (see
  [the fallback logic](/docs/reference/public-delivery-api#record-pinned-urls))

`static` is `null` for videos without public delivery enabled. `latestRecord`
carries the render status even for private videos, but its URLs are then `null`.
