...
writes a ScreenCI project into a `/screenci` directory, installs dependencies,
and can also add a GitHub Actions workflow at `.github/workflows/screenci.yaml`.
...
If you accepted the optional GitHub Actions setup, it also adds
...
You do not need to understand every file before the first run. The main one
besides the example video is:

- `screenci.config.ts` for configuring Playwright and ScreenCI.

The `videos/` directory contains the starter video source.
