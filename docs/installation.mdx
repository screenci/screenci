...
ScreenCI requires Node.js 18 or newer.
...
Run `screenci login` once first to save `SCREENCI_SECRET` into the project env
file. After that, `screenci record` records the videos, uploads them, and
renders the final output without starting an auth flow.
