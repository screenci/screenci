...
agent do it for you, see [Agent integration](/docs/agent-integration); if you
would rather not touch a repository at all, start from the web app instead
(see [Make videos by prompt](/docs/make-videos)). For how
...
ScreenCI converts these tests into product videos with the `preview` and `export` commands.
...
screenci/                   # Self-contained ScreenCI project
  ...
  .prettierrc               # Style for the source rewrites screenci makes; edit it, or delete it to disable formatting
  recordings/
...
your project's standard Playwright settings. See [Configuration](/docs/reference/configuration).
...
```bash
yarn screenci preview
```

  </TabItem>
</Tabs>

`preview` re-records every matched video, uploads the
...
record, watch, adjust, and record again without spending anything. To
narrow the run to one video,
...
without one it prints a sign-up link instead of recording (the anonymous trial
is preview-only; keep iterating for free with `screenci preview`, see
[Anonymous Trial](/docs/guides/anonymous-trial)). Signing up claims your trial
and links the project automatically on the next run.

With an account and a plan, `export` re-records anything whose sources
changed, renders, waits, and downloads the finished files into `./exports/`.
Exporting renders the video in the service and spends export minutes; higher
tiers raise export and active-video limits.
