...
- **Authored in code**: every render option (crop padding, background, frame,
  overlays) lives in your script via `screenshot.renderOptions()`, so a still's
  look is reviewed and versioned like the rest of your code.

A screenshot is captured directly from the page (no video is recorded), then
served from that hosted URL with the same versioning as videos. Unlike videos,
stills have no browser editor: change a render option in the script and
re-export. By default the output is the bare crop. Add a `margin` and/or an output
...
a video). It is recorded as `renderOptions.screenshot.crop`, not set in config.
How it behaves depends on the target:

- A **locator** crop is bound to the element: its box re-resolves from the
  locator every time you re-record, while the `padding` around it stays under
  your control in the `crop()` call.
- An **explicit region** crop has no element to track, so it stays exactly the
  rectangle you passed.
...
so a still that never touches the mouse never shows one.
...
Every visual choice is a render option, set in your script. The
