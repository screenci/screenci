...
Run these from inside the generated `screenci/` directory (that is where the
ScreenCI project and its dependencies live):
...
- `recordings/*.screenci.ts` still compile without deprecated imports
...
- served/latest version behavior still matches your release workflow
...
- make sure generated scripts still target `screenci/recordings/*.screenci.ts` and use the current ScreenCI helpers
