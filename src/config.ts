...
export function defineConfig(config: ScreenCIConfig): ExtendedScreenCIConfig {
  ...
  // enableCaptureAudio flag, not the per-video captureAudio gain. captureAudio
  // is Linux-only, so warn once at run start on other platforms; parallel runs
  // need no warning because each worker captures its own isolated null sink.
  if (isRecording && config.enableCaptureAudio) {
    process.env[CAPTURE_AUDIO_ENV] = '1'
    const unsupported = screenAudioUnsupportedMessage()
    if (unsupported !== null && process.env.TEST_WORKER_INDEX === undefined) {
