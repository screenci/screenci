...
describe('normalizeOptionsArg', () => {
  ...
  it('splits a language-major object into default + per-language overrides', () => {
    ...
    expect(n.studio).toBe(false)
  })

  it('marks editable() as studio-owned with no seed', () => {
    const n = normalizeOptionsArg<RecordOptions>('recordOptions', editable())
    expect(n).toEqual({
      shared: null,
      byLang: {},
      languages: [],
  ...
  it('keeps the seed shape for editable({ ... }) (studio-owned)', () => {
    const n = normalizeOptionsArg<RecordOptions>(
      'recordOptions',
      editable({ default: { quality: '1080p' }, fi: { aspectRatio: '4:3' } })
    )
    expect(n.studio).toBe(true)
...
describe('combineRecordOptionsLayers', () => {
  ...
  it('keeps an editable patch web-owned, merging the config under its seed', () => {
    const combined = combineRecordOptionsLayers(
      { aspectRatio: '16:9', quality: '1080p' },
      editable({ fps: 30 })
    )
    expect(isEditableMarker(combined)).toBe(true)
    if (isEditableMarker(combined)) {
      expect(combined.seed).toEqual({
        aspectRatio: '16:9',
        quality: '1080p',
        fps: 30,
      })
