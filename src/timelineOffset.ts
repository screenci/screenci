 ...
 */
export type TimelineOffset = number | `${number}%` | string

/** Result of parsing a {@link TimelineOffset} string. */
export type ParsedTimelineOffset =
  /** From a number or timecode: a concrete absolute position in milliseconds. */
...
function invalidPosition(value: string): never {
  throw new Error(
    `invalid position '${value}'; expected milliseconds, a timecode 'm:ss(.f)'/'h:mm:ss(.f)', or '<n>%'`
  )
}
 ...
 */
export function parseTimelineOffset(
  value: TimelineOffset
): ParsedTimelineOffset {
  if (typeof value === 'number') {
    if (!Number.isFinite(value) || value < 0) {
      throw new Error(
        `invalid position '${value}'; expected a non-negative millisecond number`
      )
    }
    return { kind: 'absolute', ms: value }
  }
